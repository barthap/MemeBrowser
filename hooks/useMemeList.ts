...
export function useMemeList(reload: boolean = true): [MemeState, () => void, (filterText: string) => void] {
    ...
    const setFilter = (filterText: string) => {
        if(filterText.length < 1)
            setItems(state.memes);
        else
            setItems(state.memes.filter(meme => {
                const ft = filterText.toLowerCase();
                if(meme.name != null && meme.name.toLowerCase().indexOf(ft) >= 0) return true;
                return meme.content != null && meme.content.toLowerCase().indexOf(ft) >= 0;
            }));
    }
