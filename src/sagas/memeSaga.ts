...
const watchAddMemes = function* () {
    yield takeEvery(MemeConstants.ADD_MEMES, function* (
        action: AddMemesAction
    ) {
        try {
            const addedMemes = yield call(
                MemeRepository.addNewMemes,
...
const watchUpdateMemes = function* () {
    yield takeEvery(MemeConstants.UPDATE_MEME, function* (
        action: UpdateMemeAction
    ) {
...
const watchDeleteMemes = function* () {
    yield takeEvery(MemeConstants.DELETE_MEME, function* (
